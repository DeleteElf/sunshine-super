...
#include "src/display_device/device_topology.h"
#include "src/rtsp.h"
...
namespace display_device {
  ...
  namespace vdd_utils {
    ...
    // 执行虚拟驱动的管线命令，具体支持哪些命令，参考虚拟驱动的handleClient
    ...
    bool retry_with_backoff(Func check_func, const RetryConfig &config) {
      int attempt = 0;
    ...
    void update_vdd_resolution(const SingleDisplayConfiguration &config, const vdd_utils::VddSettings &vdd_settings,int displayCount=1);
     ...
     * @param displayCount
     */
    void prepare_vdd(SingleDisplayConfiguration &config, const rtsp_stream::launch_session_t &session,int displayCount=1);
