...
namespace display_device {
  ...
  NLOHMANN_JSON_SERIALIZE_ENUM(HdrState, { { HdrState::Disabled, "disabled" },
                                              { HdrState::Enabled, "enabled" } })

  /**
   * @brief Ordered map of [DEVICE_ID -> hdr_state_e].
   */
  using hdr_state_map_t = std::map<std::string, HdrState>;
  ...
  struct device_info_t {
    ...
    HdrState hdr_state; /**< Device's HDR state. @see hdr_state_e */
  };
   ...
   * ```
   */
  device_info_map_t enum_available_devices();

  std::string find_one_of_the_available_devices(const std::string &device_id);

  std::string find_device_by_friendlyname(const std::string &friendly_name);
   ...
   * ```
   */
  std::string get_display_name(const std::string &device_id);

  std::string get_display_friendly_name(const std::string &device_id);
   ...
   * ```
   */
  device_display_mode_map_t get_current_display_modes(const std::unordered_set<std::string> &device_ids);
   ...
   * ```
   */
  bool set_display_modes(const device_display_mode_map_t &modes);
   ...
   * ```
   */
  bool is_primary_device(const std::string &device_id);
   ...
   * ``
   */
  bool set_as_primary_device(const std::string &device_id);
   ...
   * ```
   */
  hdr_state_map_t get_current_hdr_states(const std::unordered_set<std::string> &device_ids);
   ...
   * ```
   */
  bool set_hdr_states(const hdr_state_map_t &states);
   ...
   * ```
   */
  active_topology_t get_current_topology();
   ...
   * ```
   */
  bool is_topology_valid(const active_topology_t &topology);
   ...
   * ```
   */
  bool is_topology_the_same(const active_topology_t &topology_a, const active_topology_t &topology_b);
...
//   *
//   */
//  bool apply_hdr_profile(const std::string &client_name);
