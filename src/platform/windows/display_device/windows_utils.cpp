...
namespace display_device::w_utils {
  ...
  HdrState get_hdr_state(const DISPLAYCONFIG_PATH_INFO &path) {
    if (!is_active(path)) {
    ...
    return (color_info.advancedColorSupported && color_info.advancedColorEnabled) ? HdrState::Enabled : HdrState::Disabled ;
  }

  bool set_hdr_state(const DISPLAYCONFIG_PATH_INFO &path, bool enable) {
